...
# 1. Limpa qualquer processo que tenha ficado travado de execucoes anteriores
echo "[1/4] Limpando processos antigos..."
sudo pkill -f gpio_poll 2>/dev/null
pkill -f whatsapp_api.py 2>/dev/null

# 2. Inicia o codigo Assembly em segundo plano (usando o & no final)
echo "[2/4] Iniciando leitura de Hardware (Assembly)..."
sudo ./build/bin/gpio_poll &
