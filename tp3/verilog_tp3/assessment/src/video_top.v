...
// ----------------------------------------------------
// MÓDULO DE VISÃO COMPUTACIONAL: DETECTOR DE POTE VAZIO
// ----------------------------------------------------
reg [31:0] contador_fundo_pote;
reg alerta_vazio;
reg rgb_vs_prev; // NOVO: Registrador para guardar o passado do VSYNC

always @(posedge pix_clk) begin
    // Atualiza a memória: O que é presente hoje, será passado no próximo clock
    rgb_vs_prev <= rgb_vs;

    // 1. DETECTOR DE BORDA
    if (rgb_vs == 1'b1 && rgb_vs_prev == 1'b0) begin
        // META REDUZIDA: Se achar mais de 20.000 pixels claros, o pote está vazio
        if (contador_fundo_pote > 20000) begin
            alerta_vazio <= 1'b1; 
        end else begin
            alerta_vazio <= 1'b0; 
        end
        ...
        // SENSIBILIDADE AUMENTADA: Valor > 100 já é considerado claro
...
// Ligamos o resultado lógico diretamente ao pino físico do LED da placa / ESP32
