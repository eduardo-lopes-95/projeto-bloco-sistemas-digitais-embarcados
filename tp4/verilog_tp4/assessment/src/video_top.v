...
// ============================================================================
// PIPELINE DE VISÃO COMPUTACIONAL v2.0 - Dog Bowl Detector
// ============================================================================
// Arquitetura:
//   Frame Buffer → [1] RGB→Gray → [2] ROI → [3] Erosão → [4] Detecção → [5] Overlay → HDMI
//
// Melhorias sobre v1.0:
//   - Conversão de cor correta (BT.601) em vez de usar canal verde bruto
//   - Região de Interesse (ROI) para ignorar fundo irrelevante
//   - Filtro morfológico (erosão) para eliminar ruído pontual
//   - Histerese temporal (dual threshold + multi-frame) anti-flickering
//   - Overlay visual com bounding box e indicador de confiança
// ============================================================================

// --- Sinais internos da pipeline de visão ---
...
// --- Estágio 1: Conversão RGB565 → Grayscale (BT.601) ---
// Usa dados RGB565 do frame buffer (off0_syn_data) durante período ativo
rgb2gray u_rgb2gray (
    .clk       (pix_clk),
    .rst_n     (hdmi_rst_n),
    .de_in     (off0_syn_de),
...
// --- Estágio 2: Região de Interesse (ROI) ---
// Foca a análise na área central onde o pote deve estar
roi_window #(
    .H_RES      (640),
    .V_RES      (480),
    .ROI_X0     (160),    // Centro horizontal: (640-320)/2
    .ROI_Y0     (120),    // Centro vertical: (480-240)/2
    .ROI_WIDTH  (320),
    .ROI_HEIGHT (240)
) u_roi_window (
    .clk       (pix_clk),
...
// --- Estágio 3: Filtro Morfológico - Erosão 3x3 ---
// Remove pixels brilhantes isolados (ruído, reflexos pontuais)
erosion_3x3 #(
    .H_RES     (640),
    .THRESHOLD (8'd100)
) u_erosion (
...
// --- Estágio 4: Motor de Detecção com Histerese ---
// Dual threshold + confirmação multi-frame para robustez
detection_engine #(
    .COUNTER_WIDTH   (20),
    .THRESHOLD_HIGH  (20'd8000),   // Pixels claros para ativar alerta
    .THRESHOLD_LOW   (20'd4000),   // Pixels claros para desativar (histerese)
    .FRAMES_TO_ALERT (4'd3),       // 3 frames consecutivos para confirmar
    .FRAMES_TO_CLEAR (4'd5)        // 5 frames consecutivos para limpar
) u_detection_engine (
    .clk              (pix_clk),
    .rst_n            (hdmi_rst_n),
    .vsync            (erode_vs),
...
// --- Estágio 5: Overlay Visual no HDMI ---
// Bounding box + indicador de status sobre a imagem original
video_overlay #(
    .H_RES        (1280),
    .V_RES        (720),
...
// LED físico: alerta do detector → pino LED / ESP32
assign O_led[0] = alerta_vazio;

// ============================================================================
// FIM DA PIPELINE DE VISÃO COMPUTACIONAL
// ============================================================================
...
(
    ...
    .I_rgb_vs      (ov_vs           ),  // VS com overlay
    .I_rgb_hs      (ov_hs           ),  // HS com overlay
    .I_rgb_de      (ov_de           ),  // DE com overlay
    .I_rgb_r       (rgb_overlay[23:16]),  // R com overlay
    .I_rgb_g       (rgb_overlay[15: 8]),  // G com overlay
    .I_rgb_b       (rgb_overlay[ 7: 0]),  // B com overlay
